...
import { upload, processUploadedFile, ensureUploadDirectories } from '../utils/fileUpload.js';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    const { page = 1, limit = 10, make, model, year, condition, minPrice, maxPrice } = req.query;
    ...
    const vehicles = await executeQuery<Vehicle & { total_count: number }>(vehiclesQuery, params);
...
router.post('/', upload.array('images', 10), async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(yearNum) || isNaN(priceNum) || isNaN(mileageNum) || isNaN(userIdNum)) {
      return res.status(400).json({
        success: false,
    ...
    const vehicleParams = [make, model, yearNum, priceNum, mileageNum, condition, description, userIdNum];
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const queries = [
      { query: 'DELETE FROM vehicle_media WHERE vehicle_id = ?', params: [vehicleId] },
