  ...
  async (req, res) => {
    ...
    try {
      ...
      for (let i = 0; i < processedFiles.length; i++) {
        ...
        const isPrimary =
          (i === 0 && existingPrimary.length === 0) || fileData.isPrimary;
      ...
      `,
        ...
        insertedFiles.push({
          ...
          thumbnailUrl: fileData.thumbnailPath
            ? getFileUrl(fileData.thumbnailPath, req)
            : null,
    ...
    const mediaFiles = mediaRows.map((media) => ({
      ...
      thumbnailUrl:
        media.file_type === "image"
          ...
          : getFileUrl(
              media.file_path.replace(/(\.[^.]+)$/, "_thumb.jpg"),
              req,
            ),
    }));
...
router.patch(
  "/vehicles/:vehicleId/media/:mediaId/primary",
  async (req, res) => {
    let connection;
...
router.delete("/vehicles/:vehicleId/media/:mediaId", async (req, res) => {
  ...
  try {
    ...
    await connection.execute("DELETE FROM vehicle_media WHERE id = ?", [
      mediaId,
    ]);
